...
- the usefulness of self-reports towards understanding the user's **subjective perspective** of the local context [@zhang2021putting];
  ...
  _Datasets_: [SmartUnitn2 dataset](https://datascientia.eu/projects/su2/). The corresponding input modalities in _DiversityOne_ are are time diaries, acceleration, screen status, airplane mode, gyroscope, ring mode, battery charge, battery level, magnetic field, doze modality,  headset plugged in, music playback, activity performed (Google Activity Recognition API), location, WiFi network connected to, proximity, WiFi networks available, Bluetooth, notifications (the following modalities are not available in _DiversityOne_: linear acceleration, gravity, rotation vector, orientation, temperature, atmospheric pressure, humidity). 
